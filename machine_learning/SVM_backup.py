...
def select_a1(data,iteration):
    ...
    return -1
    '''
    return (iteration - 1)%100 # temporary realization, above code doesnot work
...
def select_a2(data, a1_index):
    ...
    for i in range(data.length):
        if data.Earr[i] * data.Earr[a1_index] < 0.0 and abs(data.Earr[i] - data.Earr[a1_index]) > E_a2:
            print("select a2_index from value")
...
def smo(data):
    ...
    while not satisfy_stop(data):
        ...
        if(a1_index == -1 or a2_index == -1 or a1_index == a2_index or data.Earr[a1_index] == data.Earr[a2_index]):
            ...
            print_detail_info(data)
            continue
        print("a1 index:", a1_index, "  a2 index:", a2_index)
        ...
        if data.labelArr[a1_index] != data.labelArr[a2_index]:
            ...
            if max(0, a2_new) < min(a2_new, data.a[a2_index] + data.a[a1_index]): break
            a2_new = max(0, a2_new)
        ...
        if a1_new <= 1e-15: a1_new =0.0
        if a2_new <= 1e-15: a2_new =0.0
        print("a1_new:", a1_new, "  a2_new:", a2_new)
        #b.1 = -E.1 - y.1 * K(x.1,x.1) * (a.1.new - a.1.old) - y.2 * K(x.2,x.1) * (a.2.new - a.2.old) + b.old  (same to b2)
        if a1_new != 0.0 or a2_new != 0.0:
            b1_new = data.b - data.Earr[a1_index] - data.labelArr[a1_index] * (a1_new - data.a[a1_index]) * array_operate(data.dataArr[a1_index], data.dataArr[a1_index], data.gradation, 1) - data.labelArr[a2_index] * (a2_new - data.a[a2_index]) * array_operate(data.dataArr[a2_index], data.dataArr[a1_index], data.gradation, 1) 
            b2_new = data.b - data.Earr[a2_index] - data.labelArr[a1_index] * (a1_new - data.a[a1_index]) * array_operate(data.dataArr[a1_index], data.dataArr[a2_index], data.gradation, 1) - data.labelArr[a2_index] * (a2_new - data.a[a2_index]) * array_operate(data.dataArr[a2_index], data.dataArr[a2_index], data.gradation, 1) 
            if 0.0 < a1_new: data.b = b1_new
            elif 0.0 < a2_new: data.b = b2_new
            print("data.b:",data.b,"  b1_new:",b1_new,"  b2_new:",b2_new)
        data.a[a1_index] = a1_new
        data.a[a2_index] = a2_new
        #updateEi(data,a1_index);
        #updateEi(data,a2_index);
        updateEk(data)
        print_detail_info(data)
