...
def main():
    ...
    rlts = []
    for i in range(20):
        bdt = boosting_decision_tree(training_data, training_label, weight, 1)
        ...
        for j in range(len(test_data)):
            predict_y.append(bdt.predict(test_data[j]))
        wrong_index = np.argwhere(np.array(predict_y) != test_label)
        #print("wrong index:", len(wrong_index), " ", wrong_index)
        predict_y = [-1 if x == 0 else x for x in predict_y]
        rlts.append(predict_y)
        error_weight = bdt.min_weight
        ai = math.log((1.0 - error_weight)/error_weight) / 2.0
        a_arr = np.append(a_arr,ai)
        print("a_arr:",a_arr)
        denominator = 2.0 * math.sqrt(error_weight * (1.0 - error_weight))
        #print("error_weight:",error_weight, " ai:", ai)
        for j in range(len(training_data)):
            if j in wrong_index:
                ...
                weight[j] = weight[j] * math.exp(-ai) / denominator
        print("weight:",weight)
        rlts_sum = np.sum((a_arr * np.array(rlts).T).T, axis=0)
        #print(rlts_sum)
        wrong_index_2 = []
        for i in range(len(test_label)):
            if rlts_sum[i] * (test_label[i] - 0.5) <= 0:
                wrong_index_2.append(i)
        #print("wrong index 2:", len(wrong_index_2), " ", wrong_index_2)
        print(1.0 - 1.0 * len(wrong_index_2) / len(test_label))
        print("-"*50)
