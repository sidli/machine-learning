...
def SVM(train_data,train_label,data_test,label_test):
...
def main():
    ...
    data_test,label_test = read_data("sonar_valid.csv")
    ...
    for k in range(6):
        ...
        test_data = np.dot(data_test, eigenvector[:,:k+1])
        #train_data = data_covariance * eigenvector[:,k]
        SVM(train_data,label_matrix,test_data,label_test)
    print("-" * 50)
    SVM(data_matrix,label_matrix,data_test,label_test)
    SVM(np.dot(data_matrix, eigenvector),label_matrix,np.dot(data_test, eigenvector),label_test)
