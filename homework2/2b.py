...
def main():
    ...
    test_data,test_label = loadDataSet("park_train.data")
    ...
    #for c in [1.0]:
        #for d in [5.0]:
            ...
            #for i in range(1):
                #print("a.T[0] * train_label * K[i]:",np.sum(a.T[0] * train_label * K[i]))
                predict_label.append(1.0 if np.sum(a.T[0] * train_label * K[i]) + b > 0.0 else -1.0)
