...
def main():
    ...
    for c in [1.0]:
        for d in [5.0]:
            ...
            vector_index = np.array(np.where(a > toler)[0])
            print("vector_index:",vector_index)
            #print("vector_index:",vector_index)
            train_data_vector = np.vstack(train_data[vector_index])
            print(" t * a",train_label[vector_index] * a[vector_index].T[0])
            print("(Gaussian_kernel_matrix(train_data_vector, train_data, d):",K)
            print("np.sum((Gaussian_kernel_matrix(train_data_vector, train_data, d), axis=1):",np.sum(K[vector_index], axis=1))
            ...
            for i in range(len(test_data)):
                predict_label.append(1.0 if np.sum(a.T * train_label * Gaussian_kernel_matrix(train_data[[i]], train_data, d)) + b > 0.0 else -1.0)
