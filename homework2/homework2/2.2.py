...
class tree():
    ...
    def train(self):
        ...
        for key in np.unique(self.datas[:,select]):
...
def main():
    ...
    column_names = np.array(["cap-shape","cap-surface","cap-color","bruises","odor","gill-attachment","gill-spacing","gill-size","gill-color","stalk-shape","stalk-root","stalk-surface-above-ring","stalk-surface-below-ring","stalk-color-above-ring","stalk-color-below-ring","veil-type","veil-color","ring-number","ring-type","spore-print-color","population","habitat"])
    root = tree(train_data,train_label,column_names)
    #for i in range(10)
    trees = [root]
    iteration = 0
    while(len(trees) != 0):
